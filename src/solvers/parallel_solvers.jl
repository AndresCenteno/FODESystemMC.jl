...
function MCSolver(problem::FODESystem,init_state::Int,::SharedSamples;nsims::Int=Int(1e6))
    ...
    # allocation
    uiT = SharedArray(nsims)
    duiTdα = SharedArray(length(α),nsims)
    duiTdA = SharedArray(size(A,1),size(A,2),nsims)
    duiTdu0 = SharedArray(length(α),nsims)
    duiTdT = SharedArray(nsims)
